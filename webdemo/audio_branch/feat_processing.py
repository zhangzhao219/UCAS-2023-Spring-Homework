...
class Feature_extractor(nn.Module):
    ...
    def forward(self, x):
        return self.scaler(self.amp_to_db(self.mel_spec(x)).clamp(min=-50, max=80))

def mixup(mels, labels):
    batch_size = data.size(0)
    c = np.random.beta(0.2, 0.2)
    perm = torch.randperm(batch_size)
    mixed_mels = c * data + (1 - c) * data[perm, :]
    mixed_labels = torch.clamp(c * labels + (1 - c) * labels[perm, :], min=0, max=1)
